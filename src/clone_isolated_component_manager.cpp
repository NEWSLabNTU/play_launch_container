...
{
  static auto fn = reinterpret_cast<dl_allocate_tls_fn>(
...
{
  static auto fn = reinterpret_cast<dl_deallocate_tls_fn>(
...
void CloneIsolatedComponentManager::add_node_to_executor(uint64_t node_id)
{
  ...
  void * stack = mmap(
    nullptr, kChildStackSize, PROT_READ | PROT_WRITE,
  ...
  {
    std::lock_guard<std::mutex> lock(children_mutex_);
    children_[node_id] = ChildInfo{
      child_pid, pidfd, exec, stack, kChildStackSize, node_id, node_name, child_tls,
      static_cast<void *>(boot)};
  }

  RCLCPP_INFO(
    get_logger(), "Spawned isolated child PID %d for node '%s' (id %lu)",
    child_pid, node_name.c_str(), static_cast<unsigned long>(node_id));
}
...
void CloneIsolatedComponentManager::remove_node_from_executor(uint64_t node_id)
{
  ...
  RCLCPP_INFO(
    get_logger(), "Removed isolated child PID %d for node '%s'",
    child.pid, child.node_name.c_str());
